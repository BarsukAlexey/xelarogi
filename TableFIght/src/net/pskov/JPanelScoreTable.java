...
public class JPanelScoreTable extends JPanel {

    boolean isThisForJudge;
    Fighting f;

    public JPanelScoreTable(boolean isThisForJudge) {
        super();
        this.isThisForJudge = isThisForJudge;
    }

    public synchronized void setFighting(Fighting f) {
        this.f = f;
    }

    /***
     * My rendering look like не очень
     *
     * @param gg
     */
    @Override
...
//            System.err.println("Слишком маленькое разрешение");
        ...
        // начало заполения заголовка ----------------------------------------------------------------------------------
        {
            ...
            if (f.getStatusFighting() == StatusFighting.finishPending ||
                    f.getStatusFighting() == StatusFighting.finishTie ||
