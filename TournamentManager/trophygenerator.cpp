    ...
    mTournamentUID(tournamentUID)
{
    ...
    if (filePath.isEmpty() == false)
    {
        ...
        for(const std::tuple<long long, int, int, long long>& x : DBUtils::get_distinct_TYPE_FK_AgeFrom_AgeTill(database, tournamentUID))
        {
            ...
            QString genderShortName = DBUtils::getField(database, "SHORTNAME", "SEXES", sex_fk);
            ...
            for (const auto & val : stdMap)
            {
                ...
                for (const long long orderUID : val.second)
                {
                    if (orderUID != 0)
                    {
                        ...
                        QString weight = DBUtils::getField(database, "WEIGHT", "ORDERS", orderUID);
                        QDate currentDate = QDate::currentDate();
                        QDate birthdate = QDate::fromString(DBUtils::getField(database, "BIRTHDATE", "ORDERS", orderUID), "yyyy-MM-dd");
                        ...
                        QString sportCategoryName = DBUtils::getField(database, "NAME", "SPORT_CATEGORIES", DBUtils::getField(database, "SPORT_CATEGORY_FK", "ORDERS", orderUID));
                        QString regionName = DBUtils::getField(database, "NAME", "REGIONS", DBUtils::getField(database, "REGION_FK", "ORDERS", orderUID));
